...
void task7(Array& array) {
	main_task(array);

	Array negative_numbers = Array();

	for (int i = 0; i < array.size(); i++) {
		auto target = array[i];
		if (target < 0) {
			negative_numbers.push_back(target);
		}
	}

	auto task_delimiter = (negative_numbers[0] + negative_numbers[negative_numbers.size() - 1]) * 2;
	auto delimiter = task_delimiter ? task_delimiter : 1;
...
void task10(Array& array) {
	main_task(array);

	Array negative_numbers = Array();

	for (int i = 0; i < array.size(); i++) {
		double target = array[i];
		if (target < 0) {
			negative_numbers.push_back(target);
		}
	}

	double adder = negative_numbers[negative_numbers.size() - 1] / 2;
